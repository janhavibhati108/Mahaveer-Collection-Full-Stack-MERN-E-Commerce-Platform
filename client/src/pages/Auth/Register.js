...
const Register = () => {
    ...
    const sendOtp = async () => {
        try {
            ...
            if (data.success) { setOtpSent(true); toast.success("OTP sent successfully"); }
    ...
    const verifyOtp = async () => {
        try {
            ...
            if (data.success) { setOtpVerified(true); toast.success("OTP Verified Successfully"); }
            else toast.error(data.message);
        } catch (error) {
            console.log(error);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res && res.data.success) { toast.success(res.data.message); navigate("/login"); }
            ...
            >
                <div className="form-container">
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className="mb-3">
                            <button type="button" className="btn btn-warning w-100" onClick={sendOtp} disabled={loading}>
                        ...
                        {otpVerified && (
                            <div className="alert alert-success">✓ Email Verified Successfully</div>
                        )}
                        ...
                        <div className="mb-3">
                            <input type="text" value={address} onChange={(e) => setAddress(e.target.value)}
                                className="form-control" placeholder="Enter Your Address" required />
                        </div>
                        <div className="mb-3">
                        ...
                        <button type="submit" className="btn btn-primary w-100" disabled={!otpVerified}>
                            REGISTER
                        </button>
                    </form>
                </div>
